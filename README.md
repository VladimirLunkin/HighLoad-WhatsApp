...
| Redis      | Сессии, временное хранение чатов | In-memory БД позволяет обеспечить высокую скорость на чтение и запись |
| PostgreSQL | Оснавная БД                      | Надежная и самая популярная реляционная БД на данный момент  |
...
business logic - основной сервис на бэкенде, может выполнять все операции пользователей. Может масштабироваться, разбиваться на более мелкие микросервисы, например, микросервис работы с файлами.

Auth - сервис отвечающий за авторизацию всех пользователей. Взаимодействует с сессионным хранилищем.

Messenger - сервис отвечающий за отправку сообщений новых сообщений. Выделен в отдельный сервис, чтобы обеспечить обслуживание огромнного потока сообщений. Сообщения временно сохраняются в in-memory базу, что ускоряет их обработку.
